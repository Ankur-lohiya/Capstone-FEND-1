...
const handleData = async () => {
    try {
        const data = {
            from: (await handlePOST('http://localhost:3000/geoAPI', {
                "input": document.getElementById('from').value || "Amsterdam"
            })).postalCodes[0],
            to: (await handlePOST('http://localhost:3000/geoAPI', {
                "input": document.getElementById('to').value || "Haarlem"
            })).postalCodes[0],
        }
        data.pic = (await handlePOST('http://localhost:3000/pixAPI', {
        ...
        data.temp = (await handlePOST('http://localhost:3000/dsAPI', {
            "lat": data.to.lat,
            "long": data.to.lng,
            "time": data.time
        })).currently
